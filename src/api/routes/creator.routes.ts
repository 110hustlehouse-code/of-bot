...
creatorRouter.get('/', async (req: AuthRequest, res: Response) => {
  try {
...
creatorRouter.post('/', async (req: AuthRequest, res: Response) => {
  try {
...
creatorRouter.put('/:id', async (req: AuthRequest, res: Response) => {
  try {
    const { personaPrompt, isActive } = req.body;

    const [updated] = await db.update(creators)
      .set({ personaPrompt, isActive })
      .where(and(eq(creators.id, req.params.id), eq(creators.agencyId, req.agencyId!)))
      .returning();
...
creatorRouter.delete('/:id', async (req: AuthRequest, res: Response) => {
  try {
